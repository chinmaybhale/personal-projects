...
Brawler::Brawler(string temp_name, int temp_level, int temp_power, int temp_star) {
  /**
...
int Brawler::get_star_count(void) {
  /**
   * get_star_count returns the amount of
   * star powers the brawler currently has.
   */

  return star_power_count;
}
...
void Brawler::PrintBrawler(void) {
  ...
  if(level < 9) {
    ...
    if(level == 9) {
      ...
      if(star_power_count == 1) {
        cout << name << " has acquired one Star Power." << endl;
      }
      else {
        cout << name << " has acquired two Star Powers." << endl;
      }
    }
    else {
